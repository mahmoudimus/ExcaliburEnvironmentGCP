...
provision-instance-ip(){
        # $3 : instance external ip
        IP="$3"
        generate-keys "$1" "$2"
        echo "$IP ansible_user=$2" > "$HOST_FILE"
        ansible-playbook --private-key "~/.ssh/$1_key" -i "$HOST_FILE" "$PLAYBOOK"
...
provision-all(){
        ...
        HOST_LIST=$(mktemp --suffix=.gcph)
        gcloud compute instances list --format="csv(name,networkInterfaces[0].accessConfigs[0].natIP)" | tail -n +2 | tr "," " " > "$HOST_LIST"
        while read line
        do
                host=($line)
                if [ -z ${host[1]} ]
                then
                        ...
                        provision-instance-ip "${host[0]}" "$1" "${host[1]}"
                fi
        done < "$HOST_LIST"
        rm "$HOST_LIST"
}
...
if [ $# -ne 1 ] && [ $# -ne 2 ]
then
        echo "Invalid number of parameters for $0"
